...
StringResMgr::StringResMgr(CalcInterface* pCalc) :
    ...
    maDefaultLocale(ascii("en"), ascii("US"), ascii(""))
{
    maCurrentLocale = maDefaultLocale;
}
...
void StringResMgr::loadStrings()
{
    ...
    try
    {
        mxStrResMgr->setDefaultLocale(maDefaultLocale);
    }
    catch (const lang::IllegalArgumentException&)
    {
        // We should never reach here since we should always have en-US locale 
        // available.
        return;
    }
    ...
    try
    {
        maCurrentLocale = getSystemLocale();
        mxStrResMgr->setCurrentLocale(maCurrentLocale, false);
    }
    catch (const lang::IllegalArgumentException&)
    {
        // Requested locale is not supported.  Set en-US as a fallback.
        mxStrResMgr->setCurrentLocale(maDefaultLocale, true);
        maCurrentLocale = maDefaultLocale;
    }
}
...
const OUString StringResMgr::getLocaleStr(const OUString& resName)
{
    ...
    try
    {
        mxStrResMgr->setCurrentLocale(maDefaultLocale, false);
        localeStr = mxStrResMgr->resolveString(resName);
    }
    catch (const Exception&)
    {
    }
    ...
    return localeStr.getLength() > 0 ? localeStr : ascii("(missing)");
}
...
OUString StringResMgr::getResNameByID(int resid)
{
    static const OUString resNameList[] = {
        ...
        // SCSOLVER_STR_SOLVE_TO_VALUE
        ascii("SolverDialog.rbSolveToValue.Label"),
        ...
        // SCSOLVER_STR_BTN_HELP
        ascii("Common.Help.Label"),
    ...
    if (static_cast<size_t>(resid) - SCSOLVER_RES_START >= sizeof(resNameList)/sizeof(resNameList[0]))
        return ascii("");
...
void StringResMgr::getPropertiesFiles(vector<PropertiesFile>& files)
{
    ...
    const sal_Unicode sep = sal_Unicode('/');
    ...
    sal_Int32 beginPos = alldirs[0].lastIndexOf(sep);
    if (beginPos < 0)
        return;
